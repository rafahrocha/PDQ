<#
	...
	===========================================================================
	.DESCRIPTION
		Script para desinstalação do 7-Zip.
    .UPDATES
        00/00/2020 - #################
#>
...
$IniFile = "$caminho_scripts\7-Zip - Uninstall.ini"
...
if ($STATUS -ne 'Falha'){
    $historico = "Verificando instalação do $produto..."

    Try {
        $Key = $ServiceDesk._GETPROGRAM('7-Zip')
        if (!($Key)) {
            Throw
        }
    }
    Catch {
        $STATUS = "Falha"
        $historico = "ERRO - O $produto não está instalado."
    }
}
