<#
	...
	===========================================================================
	.DESCRIPTION
		Script para instalação do Microsoft PowerBI.
    .UPDATES
        00/00/2020 - #################
#>
...
$inicio = Get-Date
$produto = 'Microsoft PowerBI x86'
$pacote = 'Instalação'

<#
# Define o nome do(s) arquivo(s) para download.
$Files = @"
    'PBIDesktopRS_32bits.msi'
"@ -replace '\n' -replace '    '
#>

# Busca o caminho dos arquivos no arquivo *.ini.
$IniFile = "$caminho_scripts\Microsoft Power BI_x86 - Install.ini"
...
if ($STATUS -ne 'Falha')
{
    ...
    Try {
        Start-Process -FilePath "C:\Temp\ServiceDesk\$produto\$FileName0" -ArgumentList "/qn /norestart ACCEPT_EULA=1" -Wait
        
        if (!($ServiceDesk._GETPROGRAM('Microsoft Power BI Desktop'))) {
            Throw
        }
    }
    Catch {
