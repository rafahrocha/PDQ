<#
	...
	===========================================================================
	.DESCRIPTION
		Script para instalação do PostgreSQL.
    .UPDATES
        00/00/2020 - #################
#>
...
$Files = @"
    'postgresql-9.6.10-2-windows.exe'
"@ -replace '\n' -replace '    '
#>

# Busca o caminho dos arquivos no arquivo *.ini.
$IniFile = "$caminho_scripts\PostgreSQL - Install.ini"
...
if ($STATUS -ne 'Falha')
{
    ...
    Try {
        ...
        Start-Process -FilePath "C:\Temp\ServiceDesk\$produto\$FileName0" -ArgumentList "--unattendedmodeui none --mode unattended" -Wait
        
        if (!($ServiceDesk._GETPROGRAM('PostgreSQL'))) {
            Throw
        }
    }
    Catch {
