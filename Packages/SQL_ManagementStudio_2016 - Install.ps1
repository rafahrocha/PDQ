<#
	...
	===========================================================================
	.DESCRIPTION
		Script para instalação do SQL Management Studio 2016.
    .UPDATES
        00/00/2020 - #################
#>
...
$inicio = Get-Date
$produto = 'SQL_ManagementStudio_2016'
$pacote = 'Instalação'

<#
# Define o nome do(s) arquivo(s) para download.
$Files = @"
    'SQL_ManagementStudio_2016.exe'
"@ -replace '\n' -replace '    '
#>

# Busca o caminho dos arquivos no arquivo *.ini.
$IniFile = "$caminho_scripts\SQL_ManagementStudio_2016 - Install.ini"
...
if ($STATUS -ne 'Falha')
{
    ...
    Try {
        ...
        Start-Process -FilePath "C:\Temp\ServiceDesk\$produto\$FileName0" -ArgumentList '/install /quiet /norestart' -Wait
        
        if (!($ServiceDesk._GETPROGRAM('Microsoft SQL Server 2016'))) {
            Throw
        }
    }
    Catch {
