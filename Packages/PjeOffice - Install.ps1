<#
	...
	===========================================================================
	.DESCRIPTION
		Script para instalação do PjeOffice.
    .UPDATES
        00/00/2020 - #################
#>
...
$Files = @"
    'PJeOffice-1.0.14.exe'
"@ -replace '\n' -replace '    '
#>

# Busca o caminho dos arquivos no arquivo *.ini.
$IniFile = "$caminho_scripts\PjeOffice - Install.ini"
...
if ($STATUS -ne 'Falha')
{
    ...
    Try {
        ...
        Start-Process -FilePath "C:\Temp\ServiceDesk\$produto\$FileName0" -ArgumentList "/SP- /VERYSILENT /SUPPRESSMSGBOXES /NORESTART /CLOSEAPPLICATIONS" -Wait
        Start-Sleep -Milliseconds 200
        
        if (!($ServiceDesk._GETPROGRAM('pje-office'))) {
            Throw
        }
    }
    Catch {
