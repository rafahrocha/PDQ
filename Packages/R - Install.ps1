<#
	...
	===========================================================================
	.DESCRIPTION
		Script para instalação do R-Project.
    .UPDATES
        00/00/2020 - #################
#>
...
$IniFile = "$caminho_scripts\R - Install.ini"
...
if ($STATUS -ne 'Falha')
{
    ...
    Try {
        ...
        Start-Process -FilePath "C:\Temp\ServiceDesk\$produto\$FileName0" -ArgumentList '/SP- /VERYSILENT /NORESTART' -Wait
        
        if (!($ServiceDesk._GETPROGRAM('R for Windows'))) {
            Throw
        }
    }
    Catch {
