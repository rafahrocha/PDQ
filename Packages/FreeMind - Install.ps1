<#
	...
	===========================================================================
	.DESCRIPTION
		Script para instalação do FreeMind.
    .UPDATES
        00/00/2020 - #################
#>
...
$IniFile = "$caminho_scripts\FreeMind - Install.ini"
...
if ($STATUS -ne 'Falha')
{
    ...
    Try {
        Write-Output "Filepath0 = $FilePath0"
...
if ($STATUS -ne 'Falha')
{
    ...
    Try {
        ...
        Start-Process -FilePath "C:\Temp\ServiceDesk\$produto\$FileName0" -ArgumentList '/SP- /SUPPRESSMSGBOXES /VERYSILENT /NORESTART' -Wait
        
        if (!($ServiceDesk._GETPROGRAM('FreeMind'))) {
            Throw
        }
    }
    Catch {
