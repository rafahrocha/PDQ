<#
	...
	===========================================================================
	.DESCRIPTION
		Script para instalação do Adobe Acrobat Reader.
    .UPDATES
        00/00/2020 - #################
#>
...
$Files = @"
    'AcroRdrDC1900820071_pt_BR.exe'
"@ -replace '\n' -replace '    '
#>

# Busca o caminho dos arquivos no arquivo *.ini.
$IniFile = "$caminho_scripts\Adobe_Acrobat_Reader - Install.ini"
...
if ($STATUS -ne 'Falha')
{
    ...
    Try {
        ...
        Start-Process -FilePath "C:\Temp\ServiceDesk\$produto\$FileName0" -ArgumentList "/sAll /rs" -Wait
        Start-Sleep -Milliseconds 200
        
        if (!($ServiceDesk._GETPROGRAM('Adobe Acrobat Reader DC'))) {
            Throw
        }
    }
    Catch {
