<#
	...
	===========================================================================
	.DESCRIPTION
		Script para desinstalação do Python.
    .UPDATES
        00/00/2020 - #################
#>
...
$IniFile = "$caminho_scripts\Python - Install.ini"
...
if ($STATUS -ne 'Falha')
{
    ...
    $historico = "Desinstalando $FileName0..."

    Try {
        Start-Process -FilePath "C:\Temp\ServiceDesk\$produto\$FileName0" -ArgumentList "/uninstall /quiet" -Wait -Verb RunAs
        
        if ($ServiceDesk._GETPROGRAM('Python')) {
            Throw
        }
    }
    Catch {
        $STATUS = "Falha"
        $historico = "ERRO - Não foi possível realizar a desinstalação."
    }
}
