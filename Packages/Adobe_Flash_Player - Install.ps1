<#
	...
	===========================================================================
	.DESCRIPTION
		Script para instalação do Adobe Flash Player.
    .UPDATES
        00/00/2020 - #################
#>
...
$Files = @"
    'Adobe_Flash_Player_v15.0.0.239_R002.EXE'
"@ -replace '\n' -replace '    '
#>

# Busca o caminho dos arquivos no arquivo *.ini.
$IniFile = "$caminho_scripts\Adobe_Flash_Player - Install.ini"
...
if ($STATUS -ne 'Falha')
{
    ...
    Start-Job -Name 'CloseFirefox' -ScriptBlock {
        While($true){
    ...
    Try {
        ...
        if ($FileName0 -like "*.msi"){
            ...
            Start-Process -FilePath "C:\Temp\ServiceDesk\$produto\$FileName0" -ArgumentList '-install' -Wait
        }
        Start-Sleep -Seconds 2

        if (!($ServiceDesk._GETPROGRAM('Adobe Flash Player'))) {
            Throw
        }
    }
    Catch {
        $STATUS = "Falha"
        $historico = "ERRO - Não foi possível realizar a instalação do Adobe Flash Player."
    }
}
