<#
	...
	===========================================================================
	.DESCRIPTION
		Script para instalação do Google Chrome.
    .UPDATES
        00/00/2020 - #################
#>
...
$Files = @"
    'GoogleChrome_v62_R001.exe'
"@ -replace '\n' -replace '    '
#>

# Busca o caminho dos arquivos no arquivo *.ini.
$IniFile = "$caminho_scripts\Google Chrome - Install.ini"
...
if ($STATUS -ne 'Falha')
{
    ...
    Try {
        ...
        if (!($ServiceDesk._GETPROGRAM('Google Chrome'))) {
            Throw
        }
    }
    Catch {
