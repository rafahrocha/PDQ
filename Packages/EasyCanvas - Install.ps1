<#
	...
	===========================================================================
	.DESCRIPTION
		Script para instalação do EasyCanvas.
    .UPDATES
        00/00/2020 - #################
#>
...
$IniFile = "$caminho_scripts\EasyCanvas - Install.ini"
...
if ($STATUS -ne 'Falha')
{
    ...
    Try {
        #Start-Process -FilePath $FilePath0 -ArgumentList "-silent $FilePath1" -Wait
        Start-Process -FilePath "C:\Temp\ServiceDesk\$produto\$FileName0" -ArgumentList "-silent C:\Temp\ServiceDesk\$produto\$FileName1" -Wait
        
        if (!($ServiceDesk._GETPROGRAM('Adobe AIR'))) {
            Throw
        }
    }
    Catch {
