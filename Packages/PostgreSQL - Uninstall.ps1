<#
	...
	===========================================================================
	.DESCRIPTION
		Script para desinstalação do PostgreSQL.
    .UPDATES
        00/00/2020 - #################
#>
...
if ($STATUS -ne 'Falha') {
    ...
    Try {
        $Key = $ServiceDesk._GETPROGRAM('PostgreSQL','UString')
        if ($Key) {
            foreach ($K in $Key) {
                # Inicia o processo de desinstalação.
                ...
                Start-Process -FilePath "$K" -ArgumentList "--mode unattended" -Wait
            }

            # Verifica se o programa/sistema foi desinstalado.
            $Key = $ServiceDesk._GETPROGRAM('PostgreSQL')
            if ($Key) {
                $STATUS = "Falha"
    ...
    Catch {
        $STATUS = "Falha"
        $historico = "Uinstall_ERROR - O $produto não está instalado."
    }
}
