<#
	...
	===========================================================================
	.DESCRIPTION
		Script para instalação do Google Earth Pro.
    .UPDATES
        00/00/2020 - #################
#>
...
$inicio = Get-Date
$produto = 'Google_Earth_Pro'
$pacote = 'Instalação'

<#
# Define o nome do(s) arquivo(s) para download.
$Files = @"
    'googleearth-win-pro-7.3.0.3832.exe'
"@ -replace '\n' -replace '    '
#>

# Busca o caminho dos arquivos no arquivo *.ini.
$IniFile = "$caminho_scripts\Google_Earth_Pro - Install.ini"
...
if ($STATUS -ne 'Falha')
{
    ...
    Try {
        ...
        Start-Process -FilePath "C:\Temp\ServiceDesk\$produto\$FileName0" -ArgumentList "OMAHA=1" -Wait
        
        if (!($ServiceDesk._GETPROGRAM('Google Earth Pro'))) {
            Throw
        }
    }
    Catch {
