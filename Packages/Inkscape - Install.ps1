<#
	...
	===========================================================================
	.DESCRIPTION
		Script para instalação do Inkscape.
    .UPDATES
        00/00/2020 - #################
#>
...
$Files = @"
    'Inkscape-0.48.5-1-win32.exe'
"@ -replace '\n' -replace '    '
#>

# Busca o caminho dos arquivos no arquivo *.ini.
$IniFile = "$caminho_scripts\Inkscape - Install.ini"
...
if ($STATUS -ne 'Falha')
{
    ...
    Try {
        ...
        Start-Process -FilePath "C:\Temp\ServiceDesk\$produto\$FileName0" -ArgumentList '/S /ALLUSER=ON /DESKTOP=ON' -Wait
        
        if (!($ServiceDesk._GETPROGRAM('Inkscape'))) {
            Throw
        }
    }
    Catch {
