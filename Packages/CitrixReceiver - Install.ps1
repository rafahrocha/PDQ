<#
	...
	===========================================================================
	.DESCRIPTION
		Script para instalação do Citrix Receiver.
    .UPDATES
        00/00/2020 - #################
#>
...
$inicio = Get-Date
$produto = 'CitrixReceiver'
$pacote = 'Instalação'

<#
# Define o nome do(s) arquivo(s) para download.
$Files = @"
    'CitrixReceiver_v4.6R001.EXE'
"@ -replace '\n' -replace '    '
#>

# Busca o caminho dos arquivos no arquivo *.ini.
$IniFile = "$caminho_scripts\CitrixReceiver - Install.ini"
...
if ($STATUS -ne 'Falha')
{
    ...
    Try {
        ...
        Start-Process -FilePath "C:\Temp\ServiceDesk\$produto\$FileName0" -ArgumentList '/silent /noreboot' -Wait
        
        if (!($ServiceDesk._GETPROGRAM('Citrix Receiver'))) {
            Throw
        }
    }
    Catch {
