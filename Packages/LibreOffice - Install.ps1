<#
	...
	===========================================================================
	.DESCRIPTION
		Script para instalação e configuração do LibreOffice.
    .UPDATES
        00/00/2020 - #################
#>
...
$Files = @"
    'LibreOffice_V5.4.1.2R008.exe',
    'Extensao-LibreOffice-ComunicacaoAdministrativa_V_1_1_20171221.oxt'
"@ -replace '\n' -replace '    '
#>

# Busca o caminho dos arquivos no arquivo *.ini.
$IniFile = "$caminho_scripts\LibreOffice - Install.ini"
...
if ($STATUS -ne 'Falha')
{
    # Inicia o processo de instalação do LibreOffice.
    ...
    Try {
        ...
        if (!($ServiceDesk._GETPROGRAM('LibreOffice'))) {
            Throw
        }
    }
    Catch {
...
{
    if ($FilePath1){
        # Inicia o processo de instalação da Extensão.
        $historico = "Instalando extensão..."

        Try {
            
            Start-Process -FilePath "C:\Temp\ServiceDesk\$produto\$FileName1" -ArgumentList "/qn /norestart" -Wait
