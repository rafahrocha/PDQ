<#
	...
	===========================================================================
	.DESCRIPTION
		Script para instalação do Git_Client.
    .UPDATES
        00/00/2020 - #################
#>
...
$IniFile = "$caminho_scripts\Git - Install.ini"
...
if ($STATUS -ne 'Falha')
{
    ...
    Try {
        ...
        Start-Process -FilePath "C:\Temp\ServiceDesk\$produto\$FileName0" -ArgumentList '/SP- /VERYSILENT /SUPPRESSMSGBOXES /FORCECLOSEAPPLICATIONS' -Wait
        
        if (!($ServiceDesk._GETPROGRAM('Git version'))) {
            Throw
        }
    }
    Catch {
