<#
	...
	===========================================================================
	.DESCRIPTION
		Script para instalação do Java JDK.
    .UPDATES
        00/00/2020 - #################
#>
...
$IniFile = "$caminho_scripts\Java-JDK - Install.ini"
...
if ($STATUS -ne 'Falha')
{
    ...
    Try {
        ...
        Start-Process -FilePath "C:\Temp\ServiceDesk\$produto\$FileName0" -ArgumentList 'INSTALL_SILENT=Enable REBOOT=Disable SPONSORS=Disable' -Wait -PassThru
        Start-Sleep -Seconds 2

        if (!($ServiceDesk._GETPROGRAM('Java SE Development Kit'))) {
            Throw
        }
    }
    Catch {
