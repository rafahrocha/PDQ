<#
	...
	===========================================================================
	.DESCRIPTION
		Script para instalação do NodeJS.
    .UPDATES
        00/00/2020 - #################
#>
...
$IniFile = "$caminho_scripts\NodeJS - Install.ini"
...
if ($STATUS -ne 'Falha')
{
    ...
    Try {
        ...
        Start-Process -FilePath "C:\Windows\System32\msiexec.exe" -ArgumentList "/iC:\Temp\ServiceDesk\$produto\$FileName0 /qn /norestart" -Wait
        
        if (!($ServiceDesk._GETPROGRAM('Node.js'))) {
            Throw
        }
    }
    Catch {
